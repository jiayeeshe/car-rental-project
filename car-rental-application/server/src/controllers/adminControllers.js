    ...
    if (adminID && adminName && password && role) {
        ...
        if (existingID)
            return res.status(400).json({
                message: "The adminID Is Already Existed",
                code: "Username",
            });
        ...
        if (existingName)
            return res.status(400).json({
                message: "The adminName Is Already Existed",
                code: "Email",
            });
            ...
            if (role === "super_admin") {
                ...
                newCar
                    ...
                    .then(() => {
                        return res
                            .status(201)
                            .json({ message: "Inserted new car successfully" });
                    })
                    .catch((err) => {
                        return res.status(400).json({ error: err });
                    });
    ...
    try {
        ...
        if (admin_id && role) {
            if (role === "super_admin") {
                ...
                return res
                    .status(200)
                    .json({ message: "Car deleted successfully", removedCar });
            } else {
                return res.status(400).json("You have no such authorization");
            }
        }
