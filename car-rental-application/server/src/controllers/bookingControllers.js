    ...
    try{
        ...
        if(user_id && role){
            if(role==="User"){
                ...
                return res.status(400).json("your booking request body is problematic");
                }
...
exports.bookAndUpdateCar =  async (req, res) => {
    try{
        ...
        await bookCar(req);
        console.log(`car_id from booking: ${car}, bookingtime:${bookedTimeSlots}`);
        updatedcar = await carController.updateCarTimeSlot(car, bookedTimeSlots);
